...
function ResultPage(props) {
  ...
  const totalPoints = (questions) => {
    const result = questions.reduce((acc, item) => {
  ...
  const result = props.quizResult.map((eachQuestion, key) => {
    return (
      <TableRow key={key}>
        ...
        <TableCell colSpan={2}>
          {getCorrectAnswer(eachQuestion.options)}
        </TableCell>
      ...
      <Grid item xs={6}>
        {!props.currentUser._id ? (
          <Typography variant="body1">
            Congratulations! You have completed the quiz. Register for more
            features like creating quizes and so on.
          </Typography>
        ) : undefined}
      </Grid>
      <Grid item xs={12}>
