...
module.exports = (app) => {
    ...
    if (req.session.userID !== undefined) {
      req.session.destroy((err) => {
        if (err) {
          ...
          const hashPassword = encrypt(req.body.password);
          User.findOne({ email: req.body.email, password: hashPassword })
            .select({ password: 0 })
            .then((data) => {
  ...
  app.get('/api/currentUser', (req, res) => {
    if (req.session.userID) {
      User.findOne({ _id: req.session.userID }, { password: 0 })
        .select({ password: 0 })
        .then((data) => {
          if (!data) {
    ...
    User.findOne({ email })
      ...
      .then((tokenSaved) => {
        if (tokenSaved) {
          ...
          let userLink = `${req.protocol}://${req.get(
            'host'
          )}/resetPassword?q=${emailParameter}`;
          if (!(process.env.NODE_ENV === 'production')) {
            userLink =
              userLink.split('5000')[0] + '3000' + userLink.split('5000')[1];
          }
          const msg = {
            to: email,
  ...
  app.get('/api/verifyLink', (req, res) => {
    if (req.session.userID) {
      ...
      newToken
        .save()
        .then(() => {
          return User.findOne({ _id: req.session.userID }).select({
            password: 0,
          });
        })
        .then((data) => {
